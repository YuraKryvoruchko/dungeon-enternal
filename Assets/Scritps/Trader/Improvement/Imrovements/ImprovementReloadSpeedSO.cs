...
{
    [CreateAssetMenu(fileName = "ImrovemetnReloadSpeedSO",
                     menuName = "ScriptableObjects/Imrovemetns/ImrovemetnReloadSpeedSO",
                     order = 1)]
    public class ImrovemetnReloadSpeedSO : ImprovementSO
    {
        ...
        public override bool RunImprovement(GameObject improvementObject)
        {
            ...
            if (improvementObject.TryGetComponent(out Weapon weapon))
            {
                for (int i = 0; i < _typesOfWeapons.Length; i++)
                {
                    ...
                    {
                        if (improvementObject.TryGetComponent(out IImprovementReloadSpeed improvementWeapon))
                        {
                            switch (_wayToImprove.FunctionType)
                            {
                                case TypeWayToImprove.SetNew:
                                    improvementWeapon.SetNewReloadSpeed(Mathf.RoundToInt(_wayToImprove.Value));
                                    improvemetnRun = true;
                                    break;
                                case TypeWayToImprove.IncreaseCapacityBy:
                                    improvementWeapon.IncreaseReloadSpeedBy(Mathf.RoundToInt(_wayToImprove.Value));
                                    improvemetnRun = true;
                                    break;
                                case TypeWayToImprove.IncreaseCapacityByInPercentage:
                                    improvementWeapon.IncreaseReloadSpeedByInPercentage(_wayToImprove.Value);
                                    improvemetnRun = true;
                                    break;
                            }
                        }
