...
namespace DungeonEternal.AI
{
    public class EnemyVision : MonoBehaviour
    {
        ...
        private bool GetRaycast(Vector3 direction)
        {
            ...
            if (Physics.Raycast(startPosition, direction, out RaycastHit hit, _distance, _detectedLayer, _queryTriggerInteraction))
            {
                if (hit.transform.TryGetComponent(out HandsPlayer handsPlayer))
                {
                    result = true;
